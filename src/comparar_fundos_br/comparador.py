...
def calcula_retorno_janelas_moveis(
    cotas_diarias: pd.DataFrame, HP: int, dados_diarios_benchmark: pd.DataFrame
) -> pd.DataFrame:
...
def plotar_rentabilidade_janela_movel(df: pd.DataFrame, HP: int, benchmarks: pd.DataFrame) -> None:
    for fundo in df.columns:
        ...
        plt.legend(
            [fundo.split("//")[-1]] + benchmarks.columns.tolist(),
            loc="upper right",
            frameon=False,
...
def supera_benchmark(
    dados: pd.DataFrame,
    benchmarks: pd.DataFrame,
    HP: int,
    limit: float = 0.6,
    janela_analise: int = 252,
) -> pd.DataFrame:
    ...
    for i, fundo in enumerate(tqdm(dados.columns.tolist())):
        ...
        for bench in lista_benchmarks:
            ...
            if retorno.empty:
                break
            elif retorno.shape[0] < janela_analise:
                continue
            ...
            if percentual_sobre_bench >= limit:
                df = pd.DataFrame(
                    [fundo, percentual_sobre_bench * 100],
                    index=["Fundo", f"Supera {bench} (%)"],
                ).T.set_index("Fundo")
...
def qto_supera_benchmark(
    ...
    for i, fundo in enumerate(tqdm(dados.columns.tolist())):
        ...
        for bench in lista_benchmarks:
            ...
            df = pd.DataFrame(
                [fundo, eventos_sup, eventos_inf, em_rel_cdi],
                index=[
                    "Fundo",
                    f"% de vezes acima {bench} (%)",
...
def plotar_heatmap_rentabilidade(dados_diarios: pd.DataFrame, period: str = "M") -> None:
    ...
    """
    cmap = LinearSegmentedColormap.from_list(name="t", colors=["red", "white", "green"])
...
def plotar_heatmap_comparar_benchmark(
    dados_diarios_fundos: pd.DataFrame,
    ...
    df4[f"Ultrapassa {bench}"] = np.where(
        df4["superou"] > 0,
        100 * (df4["superou"] / (df4["superou"] + df4["nao_superou"])),
        0,
    )
